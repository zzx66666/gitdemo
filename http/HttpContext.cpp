        ...
        switch (state_){
            ...
            case HttpRequestParaseState::HEADER_VALUE:{
                if(isblank(ch)){
                    ...
                    char* curr = colon+1;
                    while (isblank(*curr)&&curr<end)++curr;
                    request_->AddHeader(std::string(start, colon), std::string(curr, end));
                    start = end + 1;
                    state_ = HttpRequestParaseState::WHEN_CR;
                }
                break;
            ...
            case HttpRequestParaseState::CR_LF_CR:{ 
                ...
                if(ch == LF){
                    ...
                    if(request_-> headers().count("Content-Length")){
                        if((contetx_length=atoi(request_->GetHeader("Content-Length").c_str()) )> 0){
                            start = end + 1;
            ...
            case HttpRequestParaseState::BODY:{
                ...
                if (end - start + 1 == contetx_length) {
                    request_->SetBody(std::string(start, end + 1));
                    state_ = HttpRequestParaseState::COMPLETE;
                }
